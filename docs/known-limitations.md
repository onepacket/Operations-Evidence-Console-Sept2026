...
- **External services are not exercised by `pnpm test`.** The suite exercises real Express routes and an isolated PostgreSQL schema, while Clerk identity, Object Storage bytes, and model completion use explicit test-only seams. It does not perform a live Clerk sign-in, presigned upload, or provider model request.
